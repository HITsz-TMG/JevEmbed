...
The four shipped model configurations set `prompts.noul_format: retrieval`. Every Noul input is encoded as a query under the fixed `Retrieve semantically similar text.` instruction. Without criteria, the question and state are separate queries, and the logit is `noul_similarity.slope * cos(question, state) + noul_similarity.intercept`. With criteria, compare the state query against two queries made from the question plus each supplied criterion, joined by a newline; the logit is `noul_criteria.slope * (cos(state, question+true) - cos(state, question+false)) + noul_criteria.intercept`. Bare `PromptConfig()` retains `legacy` for compatibility; set the prompt format explicitly in custom training configurations. The public response stays `{"type":"noul","noul":probability}` in either mode.

For criteria-free Noul, `legacy` and `retrieval` encode the same two queries in opposite order; their cosine and logit are mathematically unchanged, so an existing adapter can be used on this path. Criteria-based Noul uses different inputs across formats and must be evaluated under the selected format before comparing results. The separate `unified` option with fixed yes/no documents remains available for compatibility. Existing adapters and archived experiments keep their original prompt setting; load an existing adapter through its saved `inference.yaml` rather than attaching it to a newly changed public configuration.
...
Choice/Score logits are cosine similarities divided by their configured temperatures. Criteria-based Noul uses a true-minus-false similarity difference and `noul_criteria` slope/intercept; criteria-free Noul uses a cosine and `noul_similarity`. These scoring parameters stay fixed; training does not fit calibration parameters. The current defaults are temperature **0.1** for Choice/Score and slope **10** with intercept **0** for both Noul parameter pairs.
