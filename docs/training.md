...
`batch_size` is per GPU. Effective question batch size is `batch_size × gradient_accumulation_steps × number of GPUs`. Short accumulation windows are normalized by their actual question count across all ranks. The final distributed batch may repeat a few examples so every rank performs the same number of updates. All ranks share validation work; only the main rank writes metrics, checkpoints, and the final adapter. Resume a distributed run with the same number of GPUs. CPU training uses `--device cpu --dtype float32`. Add `--model-path ./models/<snapshot>` to use offline base weights without changing the public model configuration.
